...
class DataRetrivalTests {
  ...
  getTestDataById(a: Assert) {
    const tst = (QUnit.config as any).modules.reduce((acc: any, m: any) => {
  ...
  @test getAllModuleDataNoArgs(a: Assert) {
    ...
    a.ok(allModules[0].tests instanceof Array, 'the first item has a tests[] property');
    a.equal(typeof allModules[0].meta, 'object', 'the first item has a meta{} property');
  }

  @test getAllTestDataNoArgs(a: Assert) {
    ...
    a.equal(typeof allTests[0].testId, 'string', 'the first item has a testId property');
    a.equal(typeof allTests[0].meta, 'object', 'the first item has a meta{} property');
  }
  ...
  @test getAllModuleDataWithFilter(a: Assert) {
    ...
    a.ok(someModules[0].tests instanceof Array, 'the first item has a tests[] property');
    a.equal(typeof someModules[0].meta, 'object', 'the first item has a meta{} property');
    a.ok(someModules.length < allModules.length, 'the filtered set is samller than the full set of modules');
  }
  ...
  @test getAllTestDataWithFilter(a: Assert) {
    ...
    a.equal(typeof someTests[0].testId, 'string', 'the first item has a testId property');
    a.equal(typeof someTests[0].meta, 'object', 'the first item has a meta{} property');
  }
